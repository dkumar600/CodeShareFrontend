...
export default function EditorTab() {
  ...
  useEffect(()=>{
    if(location.pathname.split('/')[2]){
      axios.get(`https://codesharebackendapi.onrender.com/${location.pathname.split('/')[2]}`).then((res)=>setCodeSave(()=>res.data?{...res.data}:null));
  ...
  useEffect(()=>{
    if(codeSave===null && editorCode!=''){
      ...
      axios.post(`https://codesharebackendapi.onrender.com/${location.pathname.split('/')[2]}`, {
    ...
    if(editorCode!==''){
      axios.patch(`https://codesharebackendapi.onrender.com/${location.pathname.split('/')[2]}`,{
        "code": editorCode,
        "language":Lang
      })
      .then(function (response) {
